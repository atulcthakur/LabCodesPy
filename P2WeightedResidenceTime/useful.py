...
def read_aimd_pimd_xyz_faster(filename, natoms=None, comment_symbol=None):
    ...
    with open(filename, 'r') as f:  # opening the file.
        ...
        if comment_symbol is None:
            comment_symbol = list(f.readline().split()[0])[0]
            print(f" Please make sure there are no atom names in the trajectory begining with comment_symbol: {comment_symbol}. They won't be read.")
