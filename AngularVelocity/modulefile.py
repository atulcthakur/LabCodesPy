...
def read_pimd_faster(filename, natoms):
    "Read PIMD Traj Faster"
    print(f"Reading File {filename}")
    complete_traj = {}
    datatypes = {"atoms" : "str", "x" : np.float64, "y" : np.float64, "z" : np.float64}
    dataframe = pd.read_csv(filename, usecols=[0,1,2,3], delim_whitespace=True, names=['atoms','x','y','z'], comment="#", dtype=datatypes)
    dataframe = dataframe.dropna(thresh=3) 
    nframes = dataframe.shape[0] / natoms 
    dataframe_list = np.array_split(dataframe, nframes)
    zeros = np.zeros(natoms, dtype=int)
    for counter, frame in enumerate(tqdm(dataframe_list)):
        data = frame.reset_index()
        data['index'] = np.arange(natoms)
        data.insert(1,'mols', zeros)
        complete_traj[('').join(['time_',str(counter)])] = data
        #break
